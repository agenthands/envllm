...
- **Forbidden:** `time.Now()` without injection, `math/rand` global usage, goroutines without bounded semaphore, map iteration order reliance.
- **Required:** Inject clock/random if needed, deterministic iteration (sorted keys).
- **Reason:** RLM debugging requires reproducible execution traces.

### 2. Error model must be structured
All exported functions returning error MUST return a structured type:
```go
type Error struct {
    Code string
    Message string
...
```
Never return plain `fmt.Errorf()` from runtime boundaries.

### 3. Package layering must be strict
- **Allowed:** `runtime -> ops, ast, policy`, `validate -> ast, ops`, `parse -> lex, ast`, `cmd -> pkg only`.
- **Forbidden:** `runtime` importing `cmd`, `ast` importing `runtime`.
- **Reason:** Prevents circular dependencies and keeps the VM testable.

### 4. No reflection in runtime path
- **Allowed only in:** `internal/schema/`, `internal/jsonutil/`.
- **Forbidden in:** `runtime`, `ops` handlers, execution path.
- **Reason:** Reflection introduces hidden behavior and performance variability.
